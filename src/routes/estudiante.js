...
router.get("/estudiantes",security, (req, res) => {
    mysqlConnection.query('Select estudiante.id,idpersona, nombre,apellido,fecha_ingreso,carnet,status from persona Inner Join estudiante ON persona.id = estudiante.idpersona', (err, rows, fields) => {
        if (!err) {
            res.send(rows);
        } else {
            console.log(err);
