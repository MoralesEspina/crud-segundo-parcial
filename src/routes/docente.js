...
router.get("/maestros/:id", (req, res) => {
    mysqlConnection.query('Select Nombre,Apellido,fecha_nacimiento,Direccion from persona Inner Join docente ON docente.id_persona = persona.id where persona.id = ?', [req.params.id], (err, rows, fields) => {
        if (!err) {
            res.send(rows);
        } else {
            console.log(err);
